...
impl TokenTrait for TokenBDB {
    fn initialize(
        ...
        env.storage().instance().set(&DataKey::TOKEN_NAME, &name);
        env.storage().instance().set(&DataKey::TOKEN_SYMBOL, &symbol);
        env.storage().instance().set(&DataKey::DECIMALS, &decimals);
        env.storage().instance().set(&DataKey::TOTAL_SUP, &0i128);
